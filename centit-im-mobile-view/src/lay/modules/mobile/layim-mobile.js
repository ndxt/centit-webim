...
layui.define(['laytpl', 'upload', 'layer-mobile', 'zepto'], function(exports){
  ...
  //主动触发事件
  LAYIM.prototype.emit = function(obj){
    var othis = $(obj)
    let methid = othis.attr('layim-event')
    events[methid] ? events[methid].call(othis, othis) : '';
  };
  //打开一个自定义的会话界面
  ...
  var elemTpl = ['<div class="layui-layim">'
    ...
    ,'<div class="layim-tab-content">'
      ,'<ul class="layim-list-top">'
        ...
        ,'{{# } }}'
        ,'<li layim-event="newGroup"><i class="layui-icon">&#xe613;</i>新建群聊<i class="layim-new" id="LAY_layimAddGroup"></i></li>'
      ,'</ul>'
      ,'<div class="input-container">'
                  ,'<img class="search-icon" src="./src/images/search.png"/>'
                  ,'<input class="search-input" placeholder="搜索人员" id="main-query-member"/>'
                  ,'</div>'
                  ,'<div class="cut-line"></div>'
                  ,'<div class="search-list-container">'
                  ,'</div>'
      ,'<ul class="layim-list-friend2">'
  ...
  var anim = 'layui-anim-upbit', events = { 
    ...
    chat: function(othis){
      ...
      if(type === 'friend'){
        ...
        data = {
          type,
          userCode:$(othis[0]).data("id"),
          userName: othis[0].innerText,
          id: $(othis[0]).data("id"),
          name: othis[0].innerText
        }
      }
        data.name = data.name || data.username || data.groupname 
